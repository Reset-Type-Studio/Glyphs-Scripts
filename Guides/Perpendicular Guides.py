...
def calculate_tangent_for_path(nodes):
    ...
    Uses a better method for handling Bézier curves.
    """
    if len(nodes) == 2:
        # Straight segment
        ...
        # If more than two points, assume it's a Bézier segment
        mid_x, mid_y = calculate_midpoint(nodes)
        
        # Find closest node to midpoint
        closest_node = min(nodes, key=lambda node: (node.x - mid_x) ** 2 + (node.y - mid_y) ** 2)
        
        # Find next closest node (ensuring it's not the same node)
        other_nodes = [node for node in nodes if node != closest_node]
        next_closest_node = min(other_nodes, key=lambda node: (node.x - mid_x) ** 2 + (node.y - mid_y) ** 2)
        
        dx = next_closest_node.x - closest_node.x
        dy = next_closest_node.y - closest_node.y
